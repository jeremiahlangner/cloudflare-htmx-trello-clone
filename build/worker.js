...
`};function s(n,...t){return String.raw({raw:n},...t)}function T(n){let t="";do t=Math.random().toString(16).substring(2,15);while(n[t]);return t}async function d(n){return new Response(n,{headers:{"content-type":"text/html;charset=UTF-8"}})}async function $(n){return typeof n!="string"&&(n=JSON.stringify(n)),new Response(n,{headers:{"content-type":"application/json"}})}var j,E=s(j||(j=H([`
  <!doctype html>
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
...
`;function tt(){return d(s`
  ...
  `)}var _=tt;function nt(n){let{list:t}=n;return s`
    <button
  ...
  `}var k=nt;function et(n){let{list:t}=n;return s`
...
`,S=it;function rt(n){let{list:t,card:e}=n;return s`
    <div
      class="card"
      id="card-${e.id}"
      tabindex="0"
  ...
  `}var y=rt;function st(n){let t=s``;for(let e of n.cards)t+=s`${y({list:n,card:e})}`;return t}function at(n){let{lists:t}=n,e=s``;for(let r of t)e+=s`
    ...
    `;return e+=s` <div class="add-list">${S}</div> `,e}var w=at;function ot(n){let{list:t,card:e}=n;return s`
...
`};function lt(){return new Response(z.sw,{headers:{"content-type":"text/javascript;charset=UTF-8"}})}var U=lt;async function I(n){let t='[{"name":"To Do","id":"1","cards":[{"id":"2","label":"Second Card","list":"1"},{"id":"1","label":"First Card","list":"1"},{"label":"Third Card","id":"0bfdc1222adc9","list":"1"}]},{"name":"Doing","id":"2","cards":[{"id":"3","label":"First Card","list":2},{"id":"4","label":"Second Card","list":"2"}]}]';await n.env.TrelloLists.get("lists")!=t&&await n.env.TrelloLists.put("lists",'[{"name":"To Do","id":"1","cards":[{"id":"2","label":"Second Card","list":"1"},{"id":"1","label":"First Card","list":"1"},{"label":"Third Card","id":"0bfdc1222adc9","list":"1"}]},{"name":"Doing","id":"2","cards":[{"id":"3","label":"First Card","list":2},{"id":"4","label":"Second Card","list":"2"}]}]')}async function W(n){let{env:t}=n;return{lists:JSON.parse(await t.TrelloLists.get("lists"))}}async function q(n){let{route:t,env:e}=n,{list_id:r}=t.pathname.groups,i=JSON.parse(await e.TrelloLists.get("lists"));return i=i.filter(a=>a.id!==r),await e.TrelloLists.put("lists",JSON.stringify(i)),{lists:i}}async function D(n){let{request:t,env:e,route:r}=n,{list_id:i,id:a}=r.pathname.groups,o=new URLSearchParams(await t.text()),c=Object.fromEntries(o),{label:l}=c,f=JSON.parse(await e.TrelloLists.get("lists")),m=f.find(b=>b.id===i),g=m.cards.find(b=>b.id===a);return g.label=l,await e.TrelloLists.put("lists",JSON.stringify(f)),{list:m,card:g}}async function F(n){let{env:t,route:e}=n,{list_id:r,id:i}=e.pathname.groups,o=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===r),c=o.cards.find(l=>l.id===i);return{list:o,card:c}}async function B(n){let{env:t,route:e}=n,{list_id:r,id:i}=e.pathname.groups,a=JSON.parse(await t.TrelloLists.get("lists")),o=a.find(c=>c.id===r);o.cards=o.cards.filter(c=>c.id!==i),await t.TrelloLists.put("lists",JSON.stringify(a))}async function V(n){let{request:t,env:e}=n,r=new URLSearchParams(await t.text()),i=Object.fromEntries(r),{name:a}=i,o=JSON.parse(await e.TrelloLists.get("lists"));return o.push({name:a,id:T({}),cards:[]}),await e.TrelloLists.put("lists",JSON.stringify(o)),{lists:o}}async function X(n){let{request:t,route:e,env:r}=n,i=JSON.parse(await r.TrelloLists.get("lists")),{list_id:a}=e.pathname.groups,o=new URLSearchParams(await t.text()),l=Object.fromEntries(o)["label-"+a],f=i.find(g=>g.id===a),m={label:l,id:T({}),list:a};return f.cards.push(m),await r.TrelloLists.put("lists",JSON.stringify(i)),{lists:i}}async function G(n){let{env:t,route:e}=n,r=e.pathname.groups.id;return{list:JSON.parse(await t.TrelloLists.get("lists")).find(o=>o.id===r)}}async function K(n){let{env:t,route:e}=n,{list_id:r,id:i}=e.pathname.groups,o=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===r),c=o.cards.find(l=>l.id===i);return{list:o,card:c}}async function Q(n){let{request:t,env:e}=n,r=new URLSearchParams(await t.text()),i=Object.fromEntries(r),{from:a,to:o,movedCard:c,index:l,prevIndex:f}=i,[,m]=a.split("-"),[,g]=o.split("-"),b=c.replace("card-",""),p=JSON.parse(await e.TrelloLists.get("lists"));try{if(a==="board"&&o==="board"){let u=p[f];p=p.filter(v=>v.id!==u.id),p.splice(l,0,u),await e.TrelloLists.put("lists",JSON.stringify(p))}else{let u=JSON.parse(JSON.stringify(p)),v=u.find(h=>h.id===m),O=v.cards.find(h=>h.id==b);O.list=g,v.cards=v.cards.filter(h=>h.id!=b),u.find(h=>h.id==g).cards.splice(Number(l),0,O),p=u,await e.TrelloLists.put("lists",JSON.stringify(p))}}catch(u){console.error(u)}return{lists:p}}async function C(n){let{route:t,env:e}=n,r=t.pathname.groups.key;return await e.TrelloLists.get(r)}var on={async fetch(n,t,e){return new A([["/",async i=>d(E)],["/sw.js",U],["/lists",async i=>d(w(await V(i))),"POST"],["/lists/add",_],["/lists/cancel",()=>d(S)],["/lists/:list_id",async i=>d(w(await q(i))),"DELETE"],["/cards/move",async i=>d(w(await Q(i))),"POST"],["/cards/new/:list_id",async i=>d(w(await X(i))),"POST"],["/cards/cancel/:id",async i=>d(k(await G(i)))],["/cards/edit/:list_id/:id",async i=>d(R(await K(i)))],["/cards/:list_id/:id",async i=>d(y(await D(i))),"PUT"],["/cards/:list_id/:id",async i=>(await B(i),d("")),"DELETE"],["/cards/cancel-edit/:list_id/:id",async i=>d(y(await F(i)))],["/db/:key",async i=>n.headers.get("Hx-Target")==="board"?d(w(await W(i))):$(await C(i))],["/db/:key",async i=>{try{let a=await i.request.json();return t.TrelloLists.put(i.route.pathname.groups.key,JSON.stringify(a)),$(await C(i))}catch{return d("")}},"POST"]]).handle({request:n,env:t,ctx:e})},async scheduled(n,t,e){return I({event:n,env:t,ctx:e})}};export{on as default};
