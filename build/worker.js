...
`;var h={sw:`var P=Object.freeze,et=Object.defineProperty;var y=(e,t)=>P(et(e,"raw",{value:P(t||e.slice())}));var $=class{name="";store=Promise.resolve();constructor(t,i){if(console.log("setting up"),i)return i;this.name=t,this.store=new Promise((s,o)=>{let r=indexedDB.open(this.name,1);r.onerror=()=>o(r.error),r.onupgradeneeded=()=>r.result.createObjectStore(this.name),r.onsuccess=()=>s(r.result)})}async get(t){let i=await this.store;return new Promise((s,o)=>{let r=i.transaction(this.name,"readonly"),a=r.objectStore(this.name).get(t);r.oncomplete=()=>s(a.result),r.onerror=()=>o(r.error)})}async put(t,i){let s=await this.store;return new Promise((o,r)=>{let a=s.transaction(this.name,"readwrite"),n=a.objectStore(this.name).put(i,t);a.oncomplete=()=>o(n),a.onerror=()=>r(a.error)})}async delete(t){let i=await this.store;return new Promise((s,o)=>{let r=i.transaction(this.name,"readwrite"),a=r.objectStore(this.name).delete(t);r.oncomplete=()=>s(a),r.onerror=()=>o(r.error)})}},A=$;var j=class{routes;constructor(e=[]){this.routes=[];for(let t of e)this.register(...t)}register(e,t,i="GET"){this.routes.push({path:new URLPattern({pathname:e}),method:i,handler:t})}handle(e){let{request:t}=e;for(let i of this.routes){if(i.method!==t.method)continue;let s=i.path.exec({pathname:new URL(t.url).pathname});if(s)return i.handler({...e,route:s})}return new Response("Not found",{status:404})}};var H={styles:\`body{margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Oxygen,Ubuntu,Cantarell,Fira Sans,Droid Sans,Helvetica Neue,sans-serif;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}code{font-family:source-code-pro,Menlo,Monaco,Consolas,Courier New,monospace}.app{background:#3465a4;height:100vh}.header{display:flex;justify-content:center;align-items:center;background:rgba(0,0,0,.15);color:#fff;border-bottom:1px solid rgba(0,0,0,.12);font-size:40px;font-weight:200;padding:5px 0 5px 20px}.header-buttons{display:flex;margin-right:16px;margin-left:auto}.header-buttons .edit-button{margin:0 0 0 8px}.board{height:92%;display:-webkit-flex;display:flex;overflow-x:auto}.add-list{width:272px;margin:10px;-webkit-flex-shrink:0;flex-shrink:0}.add-list-button{background-color:#0000001f;border-radius:4px;cursor:pointer;color:#fff;display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;min-height:32px;padding:5px 8px;transition:background-color 85ms ease-in,opacity 40ms ease-in,border-color 85ms ease-in;height:-webkit-fit-content;height:-moz-fit-content;height:fit-content}.add-list-button:hover{background-color:#0000003d}.list{background:#dfe3e6;-webkit-flex-shrink:0;flex-shrink:0;width:272px;height:-webkit-fit-content;height:-moz-fit-content;height:fit-content;margin:10px 0 10px 10px;border-radius:4px;border:1px solid rgba(0,0,0,.12)}.list-title{cursor:pointer;padding:10px;overflow-wrap:break-word}.toggle-add-card{cursor:pointer;padding:10px;color:#333;border-radius:0 0 4px 4px;display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;border:none;width:100%}.toggle-add-card:hover{background-color:#092d4221;color:#17394d;text-decoration:underline}.card{position:relative;cursor:grab;background:#fff;margin:5px;padding:10px;border-radius:5px;border:1px solid rgba(0,0,0,.12);box-shadow:0 1px #092d4240;font-size:15px;overflow-wrap:break-word;min-height:18px}.card:hover{background:#f5f6f7}.card-icons{position:absolute;top:5px;right:5px;-webkit-flex-direction:row;flex-direction:row;-webkit-justify-content:flex-end;justify-content:flex-end}.card-icon,.card-icons{display:-webkit-flex;display:flex}.card-icon{cursor:pointer;width:24px;height:24px;-webkit-align-items:center;align-items:center;-webkit-justify-content:center;justify-content:center;border-radius:5px;margin:1px;color:#00000080;background:#f5f6f7;opacity:.9;border:none}.card-icon:hover{opacity:1;background:#dcdcdc}.edit-card .card{min-height:50px;padding-left:8px;padding-right:15px}.edit-card .card:hover{background:#fff}.edit-card-textarea{width:100%;border:none;resize:none;outline:none;font-size:15px}.edit-buttons{display:-webkit-flex;display:flex}.edit-button{cursor:pointer;box-shadow:0 1px #3f6f21;width:-webkit-fit-content;width:-moz-fit-content;width:fit-content;margin:0 5px 10px;padding:6px 12px;border-radius:5px;border:none;color:#fff;outline:none}.edit-button:hover{opacity:.7}.edit-button-cancel{cursor:pointer;margin-bottom:10px;display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center;font-size:20px;opacity:.5;outline:none;padding:0;border:none}.edit-button-cancel:hover{opacity:1}.list-title-edit{display:-webkit-flex;display:flex;-webkit-align-items:center;align-items:center}.list-title-textarea{margin:6px 0 5px 6px;border-radius:3px;border:none;resize:none;outline:none;font-size:15px;padding:5px}.list-title-textarea:focus{box-shadow:inset 0 0 0 2px #0079bf}.add-list-editor{background:#dfe3e6;border-radius:5px;padding:2px}.hidden{display:none}
\`};function d(e,...t){return String.raw({raw:e},...t)}function O(e){let t="";do t=Math.random().toString(16).substring(2,15);while(e[t]);return t}async function c(e){return new Response(e,{headers:{"content-type":"text/html;charset=UTF-8"}})}var J=H.styles,E,yt=d(E||(E=y([\`
  <!doctype html>
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
...
\`])),J),I;function M(e){return d(I||(I=y([\`
    <!doctype html>
    <html lang="en">
      <head>
        <meta charset="UTF-8" />
...
\`,C=nt;function ot(e){let{list:t,card:i}=e;return d\`
  ...
  \`}var L=ot;function at(e){let t=d\`\`;for(let i of e.cards)t+=d\`\${L({list:e,card:i})}\`;return t}function dt(e){let{lists:t}=e,i=d\`\`;for(let s of t)i+=d\`
    ...
    \`;return i+=d\` <div class="add-list">\${C}</div> \`,i}var h=dt;var U;function lt(e){return c(M({template:d(U||(U=y([\`
        <div class="app">
          <div class="header">
            htmx Trello Clone
...
\${i.label}</textarea
  ...
  \`}var B=pt;async function W(e){let{env:t}=e;return{lists:JSON.parse(await t.TrelloLists.get("lists"))}}async function F(e){let{route:t,env:i}=e,{list_id:s}=t.pathname.groups,o=JSON.parse(await i.TrelloLists.get("lists"));return o=o.filter(r=>r.id!==s),await i.TrelloLists.put("lists",JSON.stringify(o)),{lists:o}}async function V(e){let{request:t,env:i,route:s}=e,{list_id:o,id:r}=s.pathname.groups,a=new URLSearchParams(await t.text()),n=Object.fromEntries(a),{label:l}=n,p=JSON.parse(await i.TrelloLists.get("lists")),g=p.find(x=>x.id===o),f=g.cards.find(x=>x.id===r);return f.label=l,await i.TrelloLists.put("lists",JSON.stringify(p)),{list:g,card:f}}async function X(e){let{env:t,route:i}=e,{list_id:s,id:o}=i.pathname.groups,a=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===s),n=a.cards.find(l=>l.id===o);return{list:a,card:n}}async function G(e){let{env:t,route:i}=e,{list_id:s,id:o}=i.pathname.groups,r=JSON.parse(await t.TrelloLists.get("lists")),a=r.find(n=>n.id===s);a.cards=a.cards.filter(n=>n.id!==o),await t.TrelloLists.put("lists",JSON.stringify(r))}async function K(e){let{request:t,env:i}=e,s=new URLSearchParams(await t.text()),o=Object.fromEntries(s),{name:r}=o,a=JSON.parse(await i.TrelloLists.get("lists"));return a.push({name:r,id:O({}),cards:[]}),await i.TrelloLists.put("lists",JSON.stringify(a)),{lists:a}}async function Q(e){let{request:t,route:i,env:s}=e,o=JSON.parse(await s.TrelloLists.get("lists")),{list_id:r}=i.pathname.groups,a=new URLSearchParams(await t.text()),l=Object.fromEntries(a)["label-"+r],p=o.find(f=>f.id===r),g={label:l,id:O({}),list:r};return p.cards.push(g),await s.TrelloLists.put("lists",JSON.stringify(o)),{lists:o}}async function Y(e){let{env:t,route:i}=e,s=i.pathname.groups.id;return{list:JSON.parse(await t.TrelloLists.get("lists")).find(a=>a.id===s)}}async function Z(e){let{env:t,route:i}=e,{list_id:s,id:o}=i.pathname.groups,a=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===s),n=a.cards.find(l=>l.id===o);return{list:a,card:n}}async function tt(e){let{request:t,env:i}=e,s=new URLSearchParams(await t.text()),o=Object.fromEntries(s),{from:r,to:a,movedCard:n,index:l,prevIndex:p}=o,[,g]=r.split("-"),[,f]=a.split("-"),x=n.replace("card-",""),u=JSON.parse(await i.TrelloLists.get("lists"));try{if(r==="board"&&a==="board"){let m=u[p];u=u.filter(v=>v.id!==m.id),u.splice(l,0,m),await i.TrelloLists.put("lists",JSON.stringify(u))}else{let m=JSON.parse(JSON.stringify(u)),v=m.find(b=>b.id===g),N=v.cards.find(b=>b.id==x);N.list=f,v.cards=v.cards.filter(b=>b.id!=x),m.find(b=>b.id==f).cards.splice(Number(l),0,N),u=m,await i.TrelloLists.put("lists",JSON.stringify(u))}}catch(m){console.error(m)}return{lists:u}}self.addEventListener("install",e=>{});self.addEventListener("activate",e=>{});var T;self.addEventListener("fetch",e=>["https://unpkg.com/hyperscript.org","https://unpkg.com/htmx.org","https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"].includes(e.request.url)?fetch(e.request):e.respondWith((async()=>{T||(T=new A("trelloClone",T));let i=e.request,s={TrelloLists:T},o=void 0,r;try{r=JSON.parse(await s.TrelloLists.get("lists"))}catch{}if(!r){let n=await fetch("/db/lists").then(l=>l.json());await s.TrelloLists.put("lists",JSON.stringify(n))}return new j([["/",async n=>D(await W(n))],["/lists",async n=>c(h(await K(n))),"POST"],["/lists/add",q],["/lists/cancel",()=>c(C)],["/lists/:list_id",async n=>c(h(await F(n))),"DELETE"],["/cards/move",async n=>c(h(await tt(n))),"POST"],["/cards/new/:list_id",async n=>c(h(await Q(n))),"POST"],["/cards/cancel/:id",async n=>c(S(await Y(n)))],["/cards/edit/:list_id/:id",async n=>c(B(await Z(n)))],["/cards/:list_id/:id",async n=>c(L(await V(n))),"PUT"],["/cards/:list_id/:id",async n=>(await G(n),c("")),"DELETE"],["/cards/cancel-edit/:list_id/:id",async n=>c(L(await X(n)))],["/db/:key",async n=>{let l=n.route.pathname.groups.key,p=await fetch(\`/db/\${l}\`).then(g=>g.json());if(await s.TrelloLists.put(l,JSON.stringify(p)),l==="lists")return c(h({lists:p}))}],["/db/:key",async n=>{let l=n.route.pathname.groups.key,p=await s.TrelloLists.get(l);return await fetch(\`/db/\${l}\`,{headers:{"content-type":"application/json"},body:p,method:"POST"}),c("")},"POST"]]).handle({request:i,env:s,ctx:o})})()));
`};function y(){return new Response(h.sw,{headers:{"content-type":"text/javascript;charset=UTF-8"}})}var f=y;async function b(t){let e='[{"name":"To Do","id":"1","cards":[{"id":"2","label":"Second Card","list":"1"},{"id":"1","label":"First Card","list":"1"},{"label":"Third Card","id":"0bfdc1222adc9","list":"1"}]},{"name":"Doing","id":"2","cards":[{"id":"3","label":"First Card","list":2},{"id":"4","label":"Second Card","list":"2"}]}]';await t.env.TrelloLists.get("lists")!=e&&await t.env.TrelloLists.put("lists",'[{"name":"To Do","id":"1","cards":[{"id":"2","label":"Second Card","list":"1"},{"id":"1","label":"First Card","list":"1"},{"label":"Third Card","id":"0bfdc1222adc9","list":"1"}]},{"name":"Doing","id":"2","cards":[{"id":"3","label":"First Card","list":2},{"id":"4","label":"Second Card","list":"2"}]}]')}async function a(t){let{route:e,env:n}=t,r=e.pathname.groups.key;return await n.TrelloLists.get(r)}var k=new d([["/",async()=>p(u)],["/sw.js",f],["/db/:key",async t=>s(await a(t))],["/db/:key",async t=>{let e=await t.request.json();return t.env.TrelloLists.put(t.route.pathname.groups.key,JSON.stringify(e)),s(await a(t))},"POST"]]),X={async fetch(t,e,n){return k.handle({request:t,env:e,ctx:n})},async scheduled(t,e,n){return b({event:t,env:e,ctx:n})}};export{X as default};
