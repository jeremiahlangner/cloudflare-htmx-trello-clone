...
`};function a(e,...t){return String.raw({raw:e},...t)}function $(e){let t="";do t=Math.random().toString(16).substring(2,15);while(e[t]);return t}async function n(e){return new Response(e,{headers:{"content-type":"text/html;charset=UTF-8"}})}function O(e){return a`
    <!doctype html>
    <html lang="en">
      <head>
        <meta charset="UTF-8" />
...
`;function K(e){let{list:t}=e;return a`
  ...
  `}var y=K;function Q(e){let{list:t}=e;return a`
...
`,C=W;function Y(e){let{list:t,card:i}=e;return a`
    <div
      class="card"
      id="card-${i.id}"
      tabindex="0"
  ...
  `}var b=Y;function Z(e){let t=a``;for(let i of e.cards)t+=a`${b({list:e,card:i})}`;return t}function tt(e){let{lists:t}=e,i=a``;for(let s of t)i+=a`
    ...
    `;return i+=a` <div class="add-list">${C}</div> `,i}var x=tt;var E;function et(e){return n(O({template:a(E||(E=A([`
        <div class="app">
        ...
        <\/script>
      `])),x(e))}))}var N=et;function it(){return n(a`
  ...
  `)}var P=it;function rt(e){let{list:t,card:i}=e;return a`
  ...
  `}var I=rt;function st(e){let{list:t,card:i}=e;return a`
...
${i.label}</textarea
  ...
  `}var M=st;async function _(e){await e.env.TrelloLists.put("lists",'[{"name":"To Do","id":"1","cards":[{"id":"2","label":"Second Card","list":"1"},{"id":"1","label":"First Card","list":"1"},{"label":"Third Card","id":"0bfdc1222adc9","list":"1"}]},{"name":"Doing","id":"2","cards":[{"id":"3","label":"First Card","list":2},{"id":"4","label":"Second Card","list":"2"}]}]')}async function R(e){let{env:t}=e;return{lists:JSON.parse(await t.TrelloLists.get("lists"))}}async function z(e){let{request:t,env:i,route:s}=e,{list_id:r,id:d}=s.pathname.groups,o=new URLSearchParams(await t.text()),c=Object.fromEntries(o),{label:l}=c,u=JSON.parse(await i.TrelloLists.get("lists")),g=u.find(m=>m.id===r),p=g.cards.find(m=>m.id===d);return p.label=l,await i.TrelloLists.put("lists",JSON.stringify(u)),{list:g,card:p}}async function J(e){let{env:t,route:i}=e,{list_id:s,id:r}=i.pathname.groups,o=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===s),c=o.cards.find(l=>l.id===r);return{list:o,card:c}}async function j(e){let{env:t,route:i}=e,{id:s}=i.pathname.groups;return{list:JSON.parse(await t.TrelloLists.get("lists")).find(o=>o.id===s)}}async function U(e){let{env:t,route:i}=e,{list_id:s,id:r}=i.pathname.groups,d=JSON.parse(await t.TrelloLists.get("lists")),o=d.find(c=>c.id===s);o.cards=o.cards.filter(c=>c.id!==r),await t.TrelloLists.put("lists",JSON.stringify(d))}async function B(e){let{request:t,env:i}=e,s=new URLSearchParams(await t.text()),r=Object.fromEntries(s),{name:d}=r,o=JSON.parse(await i.TrelloLists.get("lists"));return o.push({name:d,id:$({}),cards:[]}),await i.TrelloLists.put("lists",JSON.stringify(o)),{lists:o}}async function D(e){let{request:t,route:i,env:s}=e,r=JSON.parse(await s.TrelloLists.get("lists")),{list_id:d}=i.pathname.groups,o=new URLSearchParams(await t.text()),l=Object.fromEntries(o)["label-"+d],u=r.find(p=>p.id===d),g={label:l,id:$({}),list:d};return u.cards.push(g),await s.TrelloLists.put("lists",JSON.stringify(r)),{list:u,card:g}}async function q(e){let{env:t,route:i}=e,s=i.pathname.groups.id;return{list:JSON.parse(await t.TrelloLists.get("lists")).find(o=>o.id===s)}}async function F(e){let{env:t,route:i}=e,{list_id:s,id:r}=i.pathname.groups,o=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===s),c=o.cards.find(l=>l.id===r);return{list:o,card:c}}async function V(e){let{request:t,env:i}=e,s=new URLSearchParams(await t.text()),r=Object.fromEntries(s),{from:d,to:o,movedCard:c}=r,[,l]=d.split("-"),[,u]=o.split("-"),g=c.replace("card-",""),p=JSON.parse(await i.TrelloLists.get("lists"));try{let m=p.find(f=>f.id===l),k=m.cards.find(f=>f.id==g);k.list=u,m.cards=m.cards.filter(f=>f.id!=g),p.find(f=>f.id==u).cards.push(k)}catch(m){console.error(m)}return await i.TrelloLists.put("lists",JSON.stringify(p)),{lists:p}}var oe={async fetch(e,t,i){return console.log(e.method,e.url),new H([["/",async r=>N(await R(r))],["/lists",async r=>n(x(await B(r))),"POST"],["/lists/add",P],["/lists/cancel",()=>n(C)],["/cards/move",async r=>n(x(await V(r))),"POST"],["/cards/new/:list_id",async r=>n(I(await D(r))),"POST"],["/cards/cancel/:id",async r=>n(y(await q(r)))],["/cards/add/:id",async r=>n(L(await j(r)))],["/cards/edit/:list_id/:id",async r=>n(M(await F(r)))],["/cards/:list_id/:id",async r=>n(b(await z(r))),"PUT"],["/cards/:list_id/:id",async r=>(await U(r),n("")),"DELETE"],["/cards/cancel-edit/:list_id/:id",async r=>n(b(await J(r)))]]).handle({request:e,env:t,ctx:i})},async scheduled(e,t,i){return _({event:e,env:t,ctx:i})}};export{oe as default};
