...
`])),w),g=y;var f={sw:`var P=Object.freeze,Z=Object.defineProperty;var L=(e,t)=>P(Z(e,"raw",{value:P(t||e.slice())}));var $=class{name="";store=Promise.resolve();constructor(t,i){if(console.log("setting up"),i)return i;this.name=t,this.store=new Promise((s,a)=>{let r=indexedDB.open(this.name,1);r.onerror=()=>a(r.error),r.onupgradeneeded=()=>r.result.createObjectStore(this.name),r.onsuccess=()=>s(r.result)})}async get(t){let i=await this.store;return new Promise((s,a)=>{let r=i.transaction(this.name,"readonly"),o=r.objectStore(this.name).get(t);r.oncomplete=()=>s(o.result),r.onerror=()=>a(r.error)})}async put(t,i){let s=await this.store;return new Promise((a,r)=>{let o=s.transaction(this.name,"readwrite"),n=o.objectStore(this.name).put(i,t);o.oncomplete=()=>a(n),o.onerror=()=>r(o.error)})}async delete(t){let i=await this.store;return new Promise((s,a)=>{let r=i.transaction(this.name,"readwrite"),o=r.objectStore(this.name).delete(t);r.oncomplete=()=>s(o),r.onerror=()=>a(r.error)})}},A=$;var H=class{routes;constructor(e=[]){this.routes=[];for(let t of e)this.register(...t)}register(e,t,i="GET"){this.routes.push({path:new URLPattern({pathname:e}),method:i,handler:t})}handle(e){let{request:t}=e;for(let i of this.routes){if(i.method!==t.method)continue;let s=i.path.exec({pathname:new URL(t.url).pathname});if(s)return i.handler({...e,route:s})}return new Response("Not found",{status:404})}};function d(e,...t){return String.raw({raw:e},...t)}function O(e){let t="";do t=Math.random().toString(16).substring(2,15);while(e[t]);return t}async function c(e){return new Response(e,{headers:{"content-type":"text/html;charset=UTF-8"}})}var k=d\`
  <svg
...
\`,C=nt;function at(e){let{list:t,card:i}=e;return d\`
    <div
      class="card"
      id="card-\${i.id}"
      tabindex="0"
  ...
  \`}var v=at;function ot(e){let t=d\`\`;for(let i of e.cards)t+=d\`\${v({list:e,card:i})}\`;return t}function dt(e){let{lists:t}=e,i=d\`\`;for(let s of t)i+=d\`
    ...
    \`;return i+=d\` <div class="add-list">\${C}</div> \`,i}var f=dt;var z;function lt(e){return c(M({template:d(z||(z=L([\`
        <div class="app">
          <div class="header">
            htmx Trello Clone
        ...
        <\\/script>
      \`])),E,f(e))}))}var _=lt;function ct(){return c(d\`
...
\${i.label}</textarea
  ...
  \`}var U=pt;async function q(e){let{env:t}=e;return{lists:JSON.parse(await t.TrelloLists.get("lists"))}}async function B(e){let{route:t,env:i}=e,{list_id:s}=t.pathname.groups,a=JSON.parse(await i.TrelloLists.get("lists"));return a=a.filter(r=>r.id!==s),await i.TrelloLists.put("lists",JSON.stringify(a)),{lists:a}}async function F(e){let{request:t,env:i,route:s}=e,{list_id:a,id:r}=s.pathname.groups,o=new URLSearchParams(await t.text()),n=Object.fromEntries(o),{label:l}=n,p=JSON.parse(await i.TrelloLists.get("lists")),m=p.find(x=>x.id===a),h=m.cards.find(x=>x.id===r);return h.label=l,await i.TrelloLists.put("lists",JSON.stringify(p)),{list:m,card:h}}async function W(e){let{env:t,route:i}=e,{list_id:s,id:a}=i.pathname.groups,o=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===s),n=o.cards.find(l=>l.id===a);return{list:o,card:n}}async function V(e){let{env:t,route:i}=e,{list_id:s,id:a}=i.pathname.groups,r=JSON.parse(await t.TrelloLists.get("lists")),o=r.find(n=>n.id===s);o.cards=o.cards.filter(n=>n.id!==a),await t.TrelloLists.put("lists",JSON.stringify(r))}async function G(e){let{request:t,env:i}=e,s=new URLSearchParams(await t.text()),a=Object.fromEntries(s),{name:r}=a,o=JSON.parse(await i.TrelloLists.get("lists"));return o.push({name:r,id:O({}),cards:[]}),await i.TrelloLists.put("lists",JSON.stringify(o)),{lists:o}}async function X(e){let{request:t,route:i,env:s}=e,a=JSON.parse(await s.TrelloLists.get("lists")),{list_id:r}=i.pathname.groups,o=new URLSearchParams(await t.text()),l=Object.fromEntries(o)["label-"+r],p=a.find(h=>h.id===r),m={label:l,id:O({}),list:r};return p.cards.push(m),await s.TrelloLists.put("lists",JSON.stringify(a)),{lists:a}}async function K(e){let{env:t,route:i}=e,s=i.pathname.groups.id;return{list:JSON.parse(await t.TrelloLists.get("lists")).find(o=>o.id===s)}}async function Q(e){let{env:t,route:i}=e,{list_id:s,id:a}=i.pathname.groups,o=JSON.parse(await t.TrelloLists.get("lists")).find(l=>l.id===s),n=o.cards.find(l=>l.id===a);return{list:o,card:n}}async function Y(e){let{request:t,env:i}=e,s=new URLSearchParams(await t.text()),a=Object.fromEntries(s),{from:r,to:o,movedCard:n,index:l,prevIndex:p}=a,[,m]=r.split("-"),[,h]=o.split("-"),x=n.replace("card-",""),u=JSON.parse(await i.TrelloLists.get("lists"));try{if(r==="board"&&o==="board"){let g=u[p];u=u.filter(y=>y.id!==g.id),u.splice(l,0,g),await i.TrelloLists.put("lists",JSON.stringify(u))}else{let g=JSON.parse(JSON.stringify(u)),y=g.find(b=>b.id===m),N=y.cards.find(b=>b.id==x);N.list=h,y.cards=y.cards.filter(b=>b.id!=x),g.find(b=>b.id==h).cards.splice(Number(l),0,N),u=g,await i.TrelloLists.put("lists",JSON.stringify(u))}}catch(g){console.error(g)}return{lists:u}}self.addEventListener("install",e=>{});self.addEventListener("activate",e=>{});var T;self.addEventListener("fetch",e=>["https://unpkg.com/hyperscript.org","https://unpkg.com/htmx.org","https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"].includes(e.request.url)?fetch(e.request):e.respondWith((async()=>{T||(T=new A("trelloClone",T));let i=e.request,s={TrelloLists:T},a=void 0,r;try{r=JSON.parse(await s.TrelloLists.get("lists"))}catch{}if(!r){let n=await fetch("/db/lists").then(l=>l.json());await s.TrelloLists.put("lists",JSON.stringify(n))}return new H([["/",async n=>_(await q(n))],["/lists",async n=>c(f(await G(n))),"POST"],["/lists/add",D],["/lists/cancel",()=>c(C)],["/lists/:list_id",async n=>c(f(await B(n))),"DELETE"],["/cards/move",async n=>c(f(await Y(n))),"POST"],["/cards/new/:list_id",async n=>c(f(await X(n))),"POST"],["/cards/cancel/:id",async n=>c(S(await K(n)))],["/cards/edit/:list_id/:id",async n=>c(U(await Q(n)))],["/cards/:list_id/:id",async n=>c(v(await F(n))),"PUT"],["/cards/:list_id/:id",async n=>(await V(n),c("")),"DELETE"],["/cards/cancel-edit/:list_id/:id",async n=>c(v(await W(n)))],["/db/:key",async n=>{let l=n.route.pathname.groups.key,p=await fetch(\`/db/\${l}\`).then(m=>m.json());if(await s.TrelloLists.put(l,JSON.stringify(p)),l==="lists")return c(f({lists:p}))}],["/db/:key",async n=>{let l=n.route.pathname.groups.key,p=await s.TrelloLists.get(l);return await fetch(\`/db/\${l}\`,{headers:{"content-type":"application/json"},body:p,method:"POST"}),c("")},"POST"]]).handle({request:i,env:s,ctx:a})})()));
